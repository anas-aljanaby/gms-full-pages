...
const TargetSelectionPanel: React.FC<TargetSelectionPanelProps> = ({ allDonors, onTargetsUpdate, languageSelection }) => {
    ...
    }, [filteredDonors, onTargetsUpdate]);

    const languageBreakdown = useMemo(() => {
        const breakdown: Record<Language, number> = { ar: 0, en: 0, tr: 0 };
        filteredDonors.forEach(donor => {
            const lang = donor.preferred_language || 'en'; // Default to 'en'
            breakdown[lang]++;
        });
        return breakdown;
    }, [filteredDonors]);
