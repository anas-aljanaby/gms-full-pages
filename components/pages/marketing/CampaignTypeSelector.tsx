...
const CampaignTypeSelector: React.FC<CampaignTypeSelectorProps> = ({ selectedType, onSelectType }) => {
    ...
    const colorClasses: Record<string, { border: string, ring: string, text: string }> = {
        'green-500': { border: 'border-green-500', ring: 'ring-green-500/20', text: 'text-green-500' },
        'blue-500': { border: 'border-blue-500', ring: 'ring-blue-500/20', text: 'text-blue-500' },
        'orange-500': { border: 'border-orange-500', ring: 'ring-orange-500/20', text: 'text-orange-500' },
        'red-500': { border: 'border-red-500', ring: 'ring-red-500/20', text: 'text-red-500' },
        'purple-500': { border: 'border-purple-500', ring: 'ring-purple-500/20', text: 'text-purple-500' },
    };
                    ...
                    <button
                        ...
                        className={`relative p-4 h-36 flex flex-col items-center justify-center text-center rounded-2xl border-2 transition-all duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-4
                            ${selectedType === type.id 
                                ? `${colors.border} ring-4 ${colors.ring} bg-white dark:bg-slate-800` 
                                : 'border-gray-200 dark:border-slate-700 bg-card dark:bg-dark-card hover:shadow-lg'
                            }`
                        }
                    >
                        <div className={colors.text}>{type.icon}</div>
