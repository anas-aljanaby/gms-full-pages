...
const WizardStepper: React.FC<WizardStepperProps> = ({ currentStep }) => {
    ...
    const steps = [
        'basics', 'audience', 'goal', 'channels', 'content', 'tracking', 'budget', 'review'
    ];
                ...
                {steps.map((stepKey, stepIdx) => {
                    ...
                    return (
                        <li key={stepKey} className={`relative ${stepIdx !== steps.length - 1 ? 'flex-1' : ''}`}>
                            {stepIdx !== 0 && (
                                <div className={`absolute left-0 top-1/2 -translate-x-full -translate-y-1/2 w-full h-0.5 ${isCompleted || isCurrent ? 'bg-primary' : 'bg-gray-300 dark:bg-slate-700'}`} aria-hidden="true" />
                            )}
                            ...
                            <div className="flex flex-col items-center text-center">
                                ...
                                <p className={`mt-2 text-xs font-semibold ${isCurrent ? 'text-primary' : 'text-gray-500'}`}>{t(`digital_marketing.wizard.step1.${stepKey}`)}</p>
                            </div>
                        </li>
                    );
                })}
