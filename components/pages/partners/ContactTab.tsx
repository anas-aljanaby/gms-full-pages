...
import type { Partner, ContactPerson } from '../../../types';
// FIX: Added missing import for PlusCircle icon
import { Mail, Phone, PlusCircle, Globe, Linkedin, Twitter, Facebook, MapPin, X as XIcon } from 'lucide-react';
import AddContactModal from '../donors_institutional/AddContactModal'; // Re-using the donor modal for consistency
...
const ContactInfoCard: React.FC<{ partner: Partner }> = ({ partner }) => {
    const { t, language } = useLocalization();
    const socialPlatforms = partner.socialMedia ? Object.entries(partner.socialMedia).filter(([, url]) => url) : [];
        ...
        <div className="bg-card dark:bg-dark-card p-6 rounded-xl shadow-inner border dark:border-slate-700/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-6">
                    <InfoItem icon={<Phone size={18} />} label={t('institutional_donors.detail.phone')} value={partner.phone || NA_Component} />
                    <InfoItem icon={<Mail size={18} />} label={t('institutional_donors.detail.email')} value={<a href={`mailto:${partner.email}`} className="text-primary hover:underline">{partner.email}</a>} />
                    <InfoItem icon={<Globe size={18} />} label={t('institutional_donors.detail.website')} value={partner.website ? <a href={partner.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{partner.website}</a> : NA_Component} />
                         ...
                         <InfoItem 
                            ...
                            value={
                                <div className="flex items-center gap-4 mt-1">
                                    {partner.socialMedia?.linkedin && <a href={partner.socialMedia.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-primary"><Linkedin /></a>}
                                    {partner.socialMedia?.twitter && <a href={partner.socialMedia.twitter} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-primary"><Twitter /></a>}
                                    {partner.socialMedia?.facebook && <a href={partner.socialMedia.facebook} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-primary"><Facebook /></a>}
                                </div>
                            } 
                        />
                    )}
                    <InfoItem icon={<MapPin size={18} />} label={t('institutional_donors.detail.address')} value={partner.address ? <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(partner.address)}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{partner.address}</a> : NA_Component} />
                </div>

                <div className={`rounded-lg overflow-hidden border dark:border-slate-700 h-64 md:h-full min-h-[250px] flex items-center justify-center bg-gray-100 dark:bg-slate-800`}>
...
const ContactTab: React.FC<{ partner: Partner }> = ({ partner }) => {
  const { t } = useLocalization();
  const toast = useToast();
  const [contacts, setContacts] = useState<ContactPerson[]>(partner.contacts || []);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleAddContact = (newContactData: Omit<ContactPerson, 'isPrimary' | 'id'> & { isPrimary?: boolean }) => {
  ...
  return (
    <div className="animate-fade-in space-y-6">
        <ContactInfoCard partner={partner} />
