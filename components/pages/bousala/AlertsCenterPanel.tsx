...
const AlertsCenterPanel: React.FC<AlertsCenterPanelProps> = ({ isOpen, onClose, goals, onCreateTask, setNotificationCount }) => {
    ...
    const handleCreateTask = (alert: SmartAlert) => {
        onCreateTask({
            title: `Corrective Task for: ${alert.goalTitle}`,
            goalId: alert.goalId,
                ...
                <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
                    ...
                    >
                        ...
                        <div className="p-4 border-b dark:border-slate-700 grid grid-cols-3 gap-2">
                            ...
                            <select value={filters.severity} onChange={e => setFilters({...filters, severity: e.target.value as any})} className="p-2 text-xs border rounded-md bg-gray-50 dark:bg-slate-800"><option value="all">{t('bousala.alertsCenter.filters.allSeverities')}</option><option value="critical">Critical</option><option value="warning">Warning</option></select>
                                ...
                                <div key={alert.id} className={`p-4 rounded-lg border-l-4 ${alert.severity === 'critical' ? 'bg-red-50 border-red-500' : 'bg-orange-50 border-orange-500'} ${reviewedAlerts.has(alert.id) ? 'opacity-60' : ''}`}>
                                    ...
                                    <div className="flex justify-end items-center gap-2 mt-3">
                                        {reviewedAlerts.has(alert.id) ? (
                                            <span className="text-xs font-bold text-green-600 flex items-center gap-1"><ShieldCheck size={14}/> Reviewed</span>
                                        ) : (
