...
const EmailTab: React.FC<{onClose: () => void}> = ({onClose}) => {
    ...
    const [form, setForm] = useState({ to: '', subject: 'Dashboard Report', message: 'Please find the attached dashboard report.' });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if(!form.to.match(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g)) {
            toast.showError('Please enter a valid email address.', {title: 'Invalid Email'});
            return;
        }
        setIsSending(true);
        setTimeout(() => {
            setIsSending(false);
            toast.showSuccess(t('shareMenu.email.sentSuccess'), {title: 'Email Sent'});
            onClose();
        }, 1500);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end pt-4">
                <button type="submit" disabled={isSending} className="flex items-center justify-center gap-2 px-4 py-2 font-semibold text-white bg-primary rounded-lg disabled:bg-gray-400">
                    ...
                    {isSending ? 'Sending...' : t('shareMenu.email.send')}
                </button>
            </div>
        </form>
    );
...
const LinkTab: React.FC<{dashboardId: string}> = ({dashboardId}) => {
    ...
    const handleCopy = () => {
        navigator.clipboard.writeText(link).then(() => {
            setIsCopied(true);
            toast.showSuccess(t('shareMenu.copySuccess'), {title: 'Copied!'});
...
const ScheduleTab: React.FC<{onClose: () => void}> = ({onClose}) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            ...
            toast.showSuccess(t('shareMenu.schedule.scheduledSuccess'), {title: 'Scheduled!'});
            onClose();
        }, 1500);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
             ...
             <FormField label={t('shareMenu.schedule.recipients')}>
                <input type="email" placeholder="email1@example.com, email2@example.com" />
            </FormField>
             <div className="flex justify-end pt-4">
                <button type="submit" disabled={isScheduling} className="flex items-center justify-center gap-2 px-4 py-2 font-semibold text-white bg-primary rounded-lg disabled:bg-gray-400">
                    ...
                    {isScheduling ? 'Scheduling...' : t('shareMenu.schedule.scheduleBtn')}
                </button>
            </div>
        </form>
    );
